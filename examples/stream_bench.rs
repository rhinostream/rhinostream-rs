...
use rhinostream::source::ScreenCap;
use rhinostream::stream::RhinoStream;
...
fn main() {
    ...
    let filter = new_nv12_filter("-r 2560x1440".parse().unwrap(), &mut ctx).unwrap();
    let config: NvencConfig = "-p p4 --profile auto --multi-pass disabled --aq disabled -t \
        low_latency -r 1920x1080 --codec hevc --color nv12 -b 10000000 -f 60".parse().unwrap();
